...
import com.shopstack.dto.order.BuyNowRequest;
...
public class OrderController {
    ...
    /** Buys a single product directly, bypassing the cart. The cart's own contents (if any) are left untouched. */
    @PostMapping("/buy-now")
    public ApiResponse<?> buyNow(@AuthenticationPrincipal UserPrincipal principal,
                                  @Valid @RequestBody BuyNowRequest request) {
        return ApiResponse.ok("Order created — proceed to payment", orderService.buyNow(principal.getId(), request));
    }
